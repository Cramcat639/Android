            ...
            public void onClick(View view) {
                pop.showYesNoDialog(
                        ...
                        (DialogInterface.OnClickListener) (dialog, which) -> {
                            String id = getIntent().getStringExtra("id");
                            try {
                                Api.eliminarUsuarioAsync(Integer.parseInt(id), new Api.ApiCallback() {
                                    @Override
                                    public void onApiResult(String result) {
                                        if (result != null) {
                                            // Manejar el resultado si es necesario
                                            // Por ejemplo, mostrar un mensaje de éxito o realizar alguna acción adicional
                                            Toast.makeText(context, "Cuenta borrada", Toast.LENGTH_LONG).show();
                                            Intent intent = new Intent(Crud.this, MainActivity.class);
                                            startActivity(intent);
                                        } else {
                                            // Manejar el caso en que la solicitud no tenga éxito
                                            Toast.makeText(context, "Error al intentar eliminar la cuenta", Toast.LENGTH_LONG).show();
                                        }
                                    }
                                });
                            } catch (JSONException e) {
